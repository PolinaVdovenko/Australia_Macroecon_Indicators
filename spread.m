...
function spread=spread(param, startyear, endyear)%year - не год, а номер года в нашем периоде (от 1 до k или от k до n)
...
%Forecast=MMY(numsteps*[startyear:endyear]);%из недель в годы
...
%пересчитываем
ForecastY=[];FactY=[];
for i=startyear:endyear-1 % из недель ќ –”√Ћя≈ћ (срзнач) в годы
...
% spread=mean(ERY);
% spread=mean(Forecast./Fact);% отношение факта к прогнозу
% spread=sum(MMY)/sum(Fact);
% spread=mean((ERY.^2)./(Forecast.^2));%ћЌ /среднеквадратическое отклонение ошибки от прогноза
%? метод макс. правдоподоби€
%MBE=max(ERY);
%MAPE_Y=mean(abs(ERY)./Fact)*100; %число, ср_отклон/факт % Mean absolute percentage error
